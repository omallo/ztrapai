...
function archive_artifacts() {
  ...
  rm -rf /storage/models/ztrapai/${RUN_NAME}
  mkdir -p /storage/models/ztrapai/${RUN_NAME}
  cp -r /artifacts/* /storage/models/ztrapai/${RUN_NAME}
}
...
python -m cProfile -o /artifacts/train.prof main.py $* 2>/artifacts/err.log | tee -a /artifacts/out.log
